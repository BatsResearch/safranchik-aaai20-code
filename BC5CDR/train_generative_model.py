...
lf = DictionaryMatcher("DictCore-Chemical", dict_core_chem, i_label="I-Chemical", uncased=True)
lf.apply(cdr_docs)
lf = DictionaryMatcher("DictCore-Chemical-Exact", dict_core_chem_exact, i_label="I-Chemical", uncased=False)
lf.apply(cdr_docs)
lf = DictionaryMatcher("DictCore-Disease", dict_core_dis, i_label="I-Disease", uncased=True)
lf.apply(cdr_docs)
lf = DictionaryMatcher("DictCore-Disease-Exact", dict_core_dis_exact, i_label="I-Disease", uncased=False)
lf.apply(cdr_docs)
...
lf = DictionaryMatcher("Element, Ion, or Isotope", terms, i_label='I-Chemical', uncased=True, match_lemmas=True)
lf.apply(cdr_docs)
...
with open('../data/umls/umls_organic_chemical.txt', 'r') as f:
    for line in f.readlines():
        terms.append(line.strip().split(" "))
lf = DictionaryMatcher("Organic Chemical", terms, i_label='I-Chemical', uncased=True, match_lemmas=True)
lf.apply(cdr_docs)
...
with open('../data/umls/umls_antibiotic.txt', 'r') as f:
    for line in f.readlines():
        terms.append(line.strip().split(" "))
lf = DictionaryMatcher("Antibiotic", terms, i_label='I-Chemical', uncased=True, match_lemmas=True)
lf.apply(cdr_docs)
...
with open('../data/umls/umls_disease_or_syndrome.txt', 'r') as f:
    for line in f.readlines():
        terms.append(line.strip().split(" "))
lf = DictionaryMatcher("Disease or Syndrome", terms, i_label='I-Disease', uncased=True, match_lemmas=True)
lf.apply(cdr_docs)
...
exceptions = {"drug", "pre", "therapy", "anesthetia", "anesthetic", "neuroleptic", "saline", "stimulus"}
...
nb = NaiveBayes(len(gen_label_to_ix)-1, len(tagging_rules), init_acc=0.9, acc_prior=0.05, balance_prior=5.0)
...
gen_label_to_ix, disc_label_to_ix = get_label_to_ix(train_data)
tagging_rules, linking_rules = get_rules(train_data)
hmm = HMM(len(gen_label_to_ix)-1, len(tagging_rules), init_acc=0.9, acc_prior=5, balance_prior=500)
