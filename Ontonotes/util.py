...
def get_tree_modifiers(tokens, is_modifier=False, depth=0):
    ...
    for token in tokens:
        ...
        if is_modifier_token or any(
                [mod in token.dep_ for mod in {'mod', 'aux', 'mark'}]):
            is_modifier = True
        else:
            if depth > 0:
                continue
...
def get_tree_subject(tokens, is_subject=False, depth=0):
    ...
    for token in tokens:
        ...
        if (is_subject or 'subj' in token.dep_) and token.pos_ in {
                'NOUN', 'PRON', 'PROPN'}:
        ...
        leaves = [token] if depth > 0 else []
        left_leaves = get_tree_subject(
            token.lefts, is_subject_token, depth + 1)
        right_leaves = get_tree_subject(
            token.rights, is_subject_token, depth + 1)
...
def get_tree_object(tokens, is_object=False, depth=0):
    ...
    for token in tokens:
        ...
        if (is_object or 'obj' in token.dep_) and token.pos_ in {
                'NOUN', 'PRON', 'PROPN'}:
        ...
        right_leaves = get_tree_object(
            token.rights, is_object_token, depth + 1)
